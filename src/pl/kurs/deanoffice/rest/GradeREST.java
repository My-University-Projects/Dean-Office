...
public class GradeREST implements GradeRepository {
	...
	public Response getById(@PathParam("id") int id) {
		try {
			...
			e.printStackTrace();
			return Response.status(Status.BAD_REQUEST).entity("Something went wrong. Grade has not been found").build();
		}
	}
	...
	public Response getGradesAverageFromSubjectWithProvidedId(@PathParam("subjectId") Integer subjectId) {
		try {
			...
			e.printStackTrace();
			return Response.status(Status.BAD_REQUEST).entity("Something went wrong. We could not compute average")
					.build();
		}
	}
